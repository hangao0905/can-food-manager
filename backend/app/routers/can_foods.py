...
from app.calculations import calculate_can_food
...
def create_can_food(can_food: CanFoodCreate, db: Session = Depends(get_db)):
    # 检查code是否已存在
    existing = db.query(CanFoodModel).filter(CanFoodModel.code == can_food.code).first()
    if existing:
        raise HTTPException(status_code=400, detail="罐头代码已存在")
    
    db_can_food = CanFoodModel(**can_food.dict())
    db.add(db_can_food)
    db.commit()
    db.refresh(db_can_food)
    
    # 计算派生字段
    calculate_can_food(db, db_can_food)
    db.commit()
    db.refresh(db_can_food)
...
def update_can_food(can_food_code: int, can_food: CanFoodUpdate, db: Session = Depends(get_db)):
    ...
    update_data = can_food.dict(exclude_unset=True)
    for field, value in update_data.items():
        setattr(db_can_food, field, value)
