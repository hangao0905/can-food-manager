...
def list_brands(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    return [{"code": b.code, "name": b.name, "alias": b.alias, "logo": b.logo, "country": b.country, "created_date": b.created_date} for b in brands]
...
def get_brand(brand_code: int, db: Session = Depends(get_db)):
    ...
    return {"code": brand.code, "name": brand.name, "alias": brand.alias, "logo": brand.logo, "country": brand.country, "created_date": brand.created_date}
...
def create_brand(data: BrandUpdate, db: Session = Depends(get_db)):
    ...
    brand = BrandModel(code=new_code, name=data.name, alias=data.alias, logo=data.logo, country=data.country or '国内', created_date=data.created_date)
    db.add(brand)
    db.commit()
    db.refresh(brand)
    return {"code": brand.code, "name": brand.name, "alias": brand.alias, "logo": brand.logo, "country": brand.country, "created_date": brand.created_date}
